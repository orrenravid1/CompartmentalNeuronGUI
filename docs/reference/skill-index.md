...
- `add-control` (kind: authoring, surface: frontend, stage: implement, trust: general): Add or update a control value contract or frontend control presentation in CompNeuroVis. Use when introducing a new value spec, changing how controls map onto frontend state, or adding a controls-panel widget. (`skills/add-control/SKILL.md`)
