...
summary: Add a WebSocket transport so a backend can run in a separate process or machine (e.g. WSL) while the vispy frontend runs on Windows. Also establishes the language-agnostic wire protocol needed for future non-Python frontends such as Unity.
---
...
`PipeTransport` uses `multiprocessing.Pipe` or a thread queue, both of which require the backend and the frontend to live in the same OS environment. That makes it impossible to run the backend on WSL (Linux, no vispy, no display) and the frontend on Windows, even though WSL2 and the Windows host share a loopback-style network interface.
...
A secondary goal is to establish the wire protocol foundation that a future non-Python frontend (e.g. Unity) would need. That frontend replaces VisPy entirely and implements its own WebSocket client in C#; the backend server is the same in both cases. This use of Unity means Unity-as-renderer. If Unity runs physics that exchanges contacts, pose, or forces with a neural simulator, that Unity process is backend-side physics infrastructure inside a composite backend, not the frontend transport.

## Goals

- Let the backend run in WSL with no display, no Qt, no vispy.
...
- Add a standalone `run_backend_server()` entry point that runs a backend loop with no Qt dependency.
...
- Keep backend implementations transport-agnostic. After the breaking rename,
  existing simulator backends should not need WebSocket-specific code.
...
- Changing the backend protocol beyond the shared backend/transport/frontend
  refactor. WebSocket should consume that protocol; it should not invent a
...
`RunSpec.transport` or equivalent runner/runtime configuration and
`WebSocketTransport` live here. This axis is about the frontend's connection to
wherever the backend runs. It should not be embedded in declarative `AppSpec`
catalogs.
...
Unity physics is a different role from Unity rendering. If Unity participates
in simulation state by exchanging actuator, contact, pose, or proprioceptive
data with a neural backend, it belongs behind a backend adapter and coupling
policy. WebSocket may still connect that composite backend to a frontend, but it
should not be the internal neural/body coupling bus.
...
### 1. Runtime `transport` field

Add a `transport` field to the run configuration. The frontend uses it when
present instead of constructing `PipeTransport` from the backend/session source:
...
class RunSpec:
    ...
    backend: Any = None
    transport: Any = None       # pass one of backend or transport, not both
...
- `backend` given, no `transport` → construct `PipeTransport(backend)` as today
...
```python
# Windows frontend
run_app(RunSpec(transport=WebSocketTransport("localhost", 8765)))

# WSL backend (no Qt, no vispy)
run_backend_server(MyBackend, host="0.0.0.0", port=8765)
```
...
If implemented after the backend/transport/frontend refactor, substitute the
thin `Message` API for `SessionCommand` and `SessionUpdate`.

`PipeTransport` already satisfies this. `WebSocketTransport` implements it. The
runtime `transport` field is typed `Transport | None`.
...
- `stop()` -> send `StopBackend`, close the socket
...
same backend loop as the pipe transport worker, sends serialized updates,
receives and dispatches serialized commands or messages. When the connection
closes or `StopBackend` arrives, shuts down cleanly.

The backend loop logic is almost identical to the current pipe worker. The main
...
No changes to protocol types, backend logic, or the frontend dispatch path.
...
The backend loop is the same; only the send/receive calls change.
...
| Runtime `transport` field + `Transport` protocol | half day |
...
- `RunSpec` or equivalent runtime config now has two mutually exclusive backend
  connection fields (`backend` and `transport`). The mutual exclusivity should be enforced at runtime with a clear
