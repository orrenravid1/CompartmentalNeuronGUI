...
def test_jaxley_scene_builder_splits_display_and_trace_fields():
    ...
    scene = JaxleyAppSpecBuilder.build_app_spec(
        geometry=geometry,
...
class DummyJaxleyBackend(JaxleyBackend):
    def build_cells(self):
        raise AssertionError("build_cells should not be called in this unit test")


def test_jaxley_session_build_app_spec_uses_sparse_trace_history_contract():
    backend = DummyJaxleyBackend()
    geometry = MorphologyGeometry(
        id="morphology",
        positions=np.zeros((2, 3), dtype=np.float32),
        orientations=np.repeat(np.eye(3, dtype=np.float32)[None, :, :], 2, axis=0),
...
def test_jaxley_session_captures_new_trace_history_on_click():
    ...
    assert updates[0].field_id == JaxleyAppSpecBuilder.HISTORY_FIELD_ID
...
def test_jaxley_session_default_scene_uses_generic_auto_morphology_coloring():
    ...
    backend.geometry = geometry
    backend._entity_index_by_id = {"seg-a": 0, "seg-b": 1}
    backend._initialize_trace_history(0.0, np.array([1.0, 2.0], dtype=np.float32))
